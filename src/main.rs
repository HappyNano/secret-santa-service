...
async fn main() -> tide::Result<()> {
    ...
    app.at("/create_user").post(create_person);
...
async fn index(req: Request<Arc<Mutex<DataBase>>>) -> tide::Result {
    let visits: usize = req.session().get("visits").unwrap();
    Ok(format!("you have visited this website {} times", visits).into())
}

async fn create_person(mut req: Request<Arc<Mutex<DataBase>>>) -> tide::Result {
    #[derive(serde::Serialize, serde::Deserialize)]
    ...
    match guard.users.values().find(
        |p| p.name == tmp_person.name
    ) {
        None => {}
        _ => {
            println!("This user exist!");
            return Ok(tide::Redirect::new("/").into());
        },
    };
